...
> **Operator note — MFA reset (v4.0):** there is no self-service or API-driven MFA reset. A user who loses both the authenticator and the recovery codes goes through the out-of-band platform-operator procedure (identity verification, then an operator soft-revoke of the `user_mfa` row). `user_mfa` carries `FORCE ROW LEVEL SECURITY` with one policy scoped `TO nc3_auth`, so the owner connection matches no policy and the reset must go through the app role, not the raw owner connection: `SET ROLE nc3_auth; SET LOCAL app.current_user = '<user-id>'; UPDATE user_mfa SET totp_secret_ciphertext = NULL, confirmed_at = NULL WHERE user_id = '<user-id>'; RESET ROLE;` inside one transaction. Check the row count before committing — a plain owner-connection `UPDATE` here silently affects zero rows and gives no error.
