...
- **`POST .../verification` and `POST .../token` require the `organization_admin` role** (B6a / US #82, per IDR-016). A successful verification names the organization and locks the domain platform-wide, so it is an admin act; reading verification state, creating assets, non-intrusive scanning, schedules and feeds stay member-level. Refusals carry problem type `org-role-required`. The gate is insider governance rather than resistance to an anonymous attacker: registration provisions the registrant as `organization_admin` of their own workspace organization, so it constrains a non-admin member of a real multi-person organization and nothing else.
...
- **Both write operations are rate-limited per `(organization, asset)`** and answer `429` with `RateLimit`, `RateLimit-Policy` and `Retry-After`. Keyed on the authenticated organization rather than client IP, which says nothing useful on an authenticated endpoint and would penalise colleagues behind one NAT.
...
- The assurance gate is **live** on `POST .../verification` and `POST .../token` since B6a (US #82). `POST .../checks` is still a mock and keeps the enforcement-free declaration (`MfaAssuranceDeclared`) until B6b (US #263) implements the DNS check, at which point it gains the live gate, the `organization_admin` role gate, and `403` in its declared responses. The `amr`-claim reading (RFC 8176 vocabulary, never `acr`) is deferred to the SSO federation phase, where it populates the same session stamp at login.
