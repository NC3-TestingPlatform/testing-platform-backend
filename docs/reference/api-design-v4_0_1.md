...
- **`POST .../checks` answers `503` with problem type `resolver-unavailable`** when no resolver is configured or the concurrency bulkhead is full. Nothing is recorded.
- **All three write operations are rate-limited** and answer `429` with `RateLimit`, `RateLimit-Policy` and `Retry-After`. and answer `429` with `RateLimit`, `RateLimit-Policy` and `Retry-After`. Keyed on the authenticated organization rather than client IP, which says nothing useful on an authenticated endpoint and would penalise colleagues behind one NAT.
