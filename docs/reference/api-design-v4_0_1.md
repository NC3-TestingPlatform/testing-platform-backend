...
- `DELETE` answers `409` while scan history, discovered children, a verification, a schedule, or a feed reference the asset.
...
- `POST .../verification` and `POST .../checks` require current MFA assurance.
- **`POST .../verification`, `POST .../token` and `POST .../checks` require the `organization_admin` role** (B6a / US #82 and B6b / US #263, per IDR-016). A successful verification names the organization and locks the domain platform-wide, so it is an admin act; reading verification state, creating assets, non-intrusive scanning, schedules and feeds stay member-level. Refusals carry problem type `org-role-required`. The gate is insider governance rather than resistance to an anonymous attacker: registration provisions the registrant as `organization_admin` of their own workspace organization, so it constrains a non-admin member of a real multi-person organization and nothing else.
- **`POST .../verification` answers `409` when the asset is not a domain.** DNS-TXT verification applies to domain assets only; the rule is cross-table, so the schema cannot express it and the application enforces it (data model §14 closing note).
- **`POST .../checks` is realized since B6b (US #263)** and carries the full gate set: `organization_admin`, live MFA assurance, and three rate windows. Its declared responses are `401`, `403`, `404`, `409`, `422`, `429` and `503`.
- **A check that ran answers `200` with a namespaced `failure_code`; a check that could not run answers `503` and records nothing.** The prefix carries the distinction a client acts on: `dns.*`, `challenge.*` and `policy.*` are outcomes the user can address (`dns.record-not-found`, `dns.name-not-found`, `dns.token-mismatch`, `dns.resolver-failure`, `dns.corroboration-not-reached`, `challenge.superseded`, `policy.zone-requires-dnssec`), and an expired token is refused with `409` rather than stamped, while `platform.*` means the fault is ours (`platform.resolver-unavailable`, `platform.capacity-exhausted`) and no `failure_code` is stamped, because telling a user their DNS is wrong when the resolver was unreachable is worse than saying nothing. A successful check clears any previous `failure_code`.
- **`zone` coverage requires a DNSSEC-validated answer; `exact` accepts an unvalidated one and records the fact** (IDR-019). Zone is the widest grant in the system and in v4.1 authorizes intrusive scanning of names that may be delegated to other legal entities, so it demands the strong proof. Requiring validation everywhere would refuse roughly nine `.lu` domains in ten — measured 9.76% signed (dns.lu, 2026-07-27) — so `exact` accepts one validated answer **or** agreement across the configured resolver quorum. A zone request over an unsigned zone answers `200` with `policy.zone-requires-dnssec`, whose message names both remedies: sign the zone, or verify with `exact` scope. **UI-06-x must surface this at scope-choice time**, not at check time, or a user picks `zone`, publishes a record and only then learns it cannot succeed.
- **`POST .../checks` answers `409` on an expired token** (`challenge.expired`): the remedy is a new token, not a retry.
- **`POST .../checks` answers `409` with problem type `domain-claim-lost` when another organization already holds the verified claim.** The body says the domain is claimed and never by whom: the conflicting row belongs to another tenant and is invisible under row-level security, and identifying it would be a cross-tenant disclosure. Releasing a claim is a platform-operator procedure in v4.0 (`docs/verification-claim-release.md`), which the refusal names so a legitimate owner has somewhere to go.
- **`POST .../checks` answers `503` with problem type `resolver-unavailable`** whenever the check could not run: no resolver is configured, a configured entry names a transport this build cannot construct, the concurrency bulkhead is full, the process has spent its outbound-DNS budget for the window, or no resolver made any statement about the name. Nothing is recorded in any of those cases — the fault is the platform's, and stamping a `dns.*` reason would tell the user their DNS is wrong when it may be fine.
- **All three write operations are rate-limited** and answer `429` with `RateLimit`, `RateLimit-Policy` and `Retry-After`. Keyed on the authenticated organization rather than client IP, which says nothing useful on an authenticated endpoint and would penalise colleagues behind one NAT.
- **The three realized operations declare the SessionCookie scheme alone.** They previously also published OpenID Connect and API key; both are unimplemented seams in v4.0 and neither can satisfy these handlers, so declaring them misled clients about how to authenticate.
- **Responses that carry a challenge token are `Cache-Control: no-store`.** The token is a value the caller is about to publish; a shared proxy or browser disk cache retaining it would outlive the reason it was shown. Same treatment as the feed-token responses in §5.2.
...
- The assurance gate is **live on all three write operations**: `POST .../verification` and `POST .../token` since B6a (US #82), and `POST .../checks` since B6b (US #263). Before B6b, `/checks` carried no assurance declaration at all — an earlier revision of this section claimed it held the enforcement-free `MfaAssuranceDeclared`, which was never true of the router. The `amr`-claim reading (RFC 8176 vocabulary, never `acr`) is deferred to the SSO federation phase, where it populates the same session stamp at login.
