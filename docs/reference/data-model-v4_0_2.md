...
- `dnssec_validated`, `resolvers` and `corroborating_answers` are written in v4.0 and read by nothing until the v4.1 intrusive gate consumes them (IDR-019). `last_reverified_at` is neither written nor read in v4.0: the column ships now so the v4.1 re-check rule needs no second migration.
...
| `purge_due_at`        | timestamptz | Not null; between the upload time and 24 hours after it          |
...
| `modules`              | `scan_module[]`   | Not null; never empty; what the launch asked for. Compare against the tasks to see what ran         |
...
| `created_by_user_id`   | UUID            | Nullable; foreign key to `app_user.id`; `ON DELETE SET NULL` |
| `modules`              | `scan_module[]` | Not null; never empty                                        |
| `module_configuration` | JSONB           | Not null; default `{}`                                       |
...
| `retention_until`     | timestamptz | Not null; default 24 months after `occurred_at`; never earlier than `occurred_at`      |
...
| `file_upload`                   | `purge_due_at >= uploaded_at`                                                                                                      | the purge deadline never precedes the upload                          |
| `scan_job`                      | `num_nonnulls(asset_id, target_domain, file_upload_id) = 1`                                                                        | exactly one launch target                                             |
| `scan_job`                      | `cardinality(modules) >= 1`                                                                                                        | a launch asks for at least one module                                 |
...
| `scan_task`                     | `(file_upload_id IS NOT NULL) = (module = 'file')`                                                                                 | a File task targets an upload, and nothing else does                  |
...
| `schedule`                      | `cardinality(modules) >= 1`                                                                                                        | a schedule runs at least one module                                   |
...
| `audit_event`                   | `retention_until >= occurred_at`                                                                                                   | retention never ends before the event occurred                        |
